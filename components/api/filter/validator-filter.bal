...
public type validateRequestFilter object {
    ...
    http:FilterContext context) returns boolean {
        ...
        string token = "";
        if(request.hasHeader(constants:AUTHERIZATION_TOKEN)) {
            string tokenHeaderValue = request.getHeader(constants:AUTHERIZATION_TOKEN);
            ...
            log:printInfo("Did not receive any token. 
